# SYL — Build Log ARCHIVE (append-only history)

> This is the full historical build log, kept for humans/auditing only. Agents do NOT need to read
> this at session start — the live `BUILDLOG.md` holds current state + recent entries + Next up.
> Older entries were rotated here on 2026-06-20 to keep the active log small (token-efficient).
> Read this only when deep-diving why/when/who on something not covered by the live log.
